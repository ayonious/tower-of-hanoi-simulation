...
public class FlickerCanvas extends Canvas implements Runnable {
    ...
    void init(int n,int p) 
    {
        ...
        for(i=1;i<=Discs;i++) stack[1][i]=i;
    	for(i=0;i<=Pegs;i++)stack_top[i]=0;
    	stack_top[1]=Discs;
        ...
        for(i=0;i<Pegs;i++)
        	stack_top[i]=0;
        for(i=0;i<Discs;i++) stack[0][stack_top[0]++]=i;//0 mane sobche boro guti  
    ...
    public void run() {
        try {
        	...
        	for(Curr_step=0;Curr_step<Total_steps;Curr_step++)
        	{
        		...
        		//uppere jaitese
        		xx=x;
        		for(yy=y;yy>=0;yy--) dumb(xx,yy,zz);
        		y=0;
        		...
        		//slide
        		yy=0;
        		for(xx=x;xx!=X;xx+=id) dumb(xx,yy,zz);
        		dumb(xx,yy,zz);
        		x=X;
        		
        		//down e ese portese
        		for(yy=0;yy<=Y;yy++) dumb(xx,yy,zz);
